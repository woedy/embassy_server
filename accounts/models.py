...
from visa.models import PersonalInfo
...
class UserManager(BaseUserManager):
    ...
    def search(self, query=None):
        ...
        if query is not None:
            or_lookup = (Q(email__icontains=query) | Q(last_name__icontains=query) | Q(first_name__icontains=query))
...
def post_save_create_user_profile_objects_receiver(sender, instance, created, *args, **kwargs):
    if created:
        personal_info = PersonalInfo.objects.create(user=instance)
    ...
    def send_activation(self):
        if not self.activated and not self.forced_expired:
            if self.key:
                ...
                base_url = getattr(settings, 'BASE_URL', 'https://www.togoembassy.com')
